...
"""
This file is for summarizing basic info about each of the JSON files for the MS MARCO dataset.
"""
...
def get_query_types(dataset):
    """
    Gets all of the query types for a given dataset, and packages them in a dict with the first index key for that query type.
    """
    ...
    for query_type in query_types_dataset.values():
        if query_type not in get_query_types:
            get_query_types.append(query_type)
    # Getting the index key of the first occurence of query type.
    result = {}
...
def get_full_dataset_sample_string(dataset, tab_n, line_n):
    ...
    for query_type in dataset_sample.keys():
        ...
        answers = interable_to_str(dataset_sample[query_type]['answers'])
        output += crop_labeled_line_to_str('Answers: ', answers, line_n, tab_n)
        # Adding the well formed answer section section, indented once.
        wellFormedAnswers = interable_to_str(dataset_sample[query_type]['wellFormedAnswers'])
...
def write_dataset_info(input_file, output_file, tab_n, line_n):
    ...
    data_categories = interable_to_str(get_data_categories(dataset))
    query_types = interable_to_str(get_query_types(dataset))
    ...
    with open(output_file, 'w+') as f:
        ...
        out = crop_labeled_line_to_str('Data categories: ', data_categories, line_n, 0)
